...
// 방문한 노드의 info를 확인
int visit(struct treeNode *t, char c) {
...
void traverse(struct treeNode *t) {
	...
	cin >> c;
	if (c == '+' || c == '*' || c == '-' || c == '/')
		cout << "연산자를 입력했습니다 프로그램을 종료합니다." << endl;
	queue.put(t);	// 새로운 원형큐에 삽입, 순회하며 출력
	while (!queue.empty()) {
		t = queue.get();

		// 해당 info값을 갖는 노드가 존재할 경우
		if (visit(t, c)) {
			cout << "Level : ";
			cout << t->level;	// 해당노드의 레벨을 출력
			return;		// 함수종료
		}

		// 마지막 노드가 아닌 경우
		if (t->l != z) {
			queue.put(t->l);	// queue[]에 왼쪽 자식노드 삽입
			t->l->level = t->level + 1;	// 왼쪽 자식노드의 레벨을 1증가시킴
		}
		if (t->r != z) {
			queue.put(t->r);	// queue[]에 오른쪽 자식노드 삽입
			t->r->level = t->level + 1;	// 오른쪽 자식노드의 레벨을 1증가시킴
		}
	}
	cout << "Not Found";	// 발견되지 않은 경우 문구 출력
}
